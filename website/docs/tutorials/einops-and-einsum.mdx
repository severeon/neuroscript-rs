...
All shape transitions are checked at compile time.
...
<InteractiveExample
  ...
  initialCode={`neuron BatchMatMul:
    ...
    graph:
        in -> (a, b)
        (a, b) -> Einsum(\`bij,bjk->bik\`) -> out`}
/>

The input is forked into `a` and `b`, then both are passed as a tuple to `Einsum`. The equation `` `bij,bjk->bik` `` contracts over the `j` index — that's matrix multiplication.
