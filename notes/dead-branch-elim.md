...
1. Extend IR for arm metadata (src/ir.rs)
  - Add is_reachable: bool to MatchArm (default true).
  - Update Display/Clone/etc. for new field.
1. Mark unreachable arms in validator (src/validator.rs)
...
1. Create optimizer pass (src/optimizer.rs - new file)
...
## 1. IR Changes
  - **src/ir.rs:129-134** - `MatchArm` struct: Add `is_reachable: bool = true;`

## 2. Validator Marking
  - **src/validator.rs:868-904** - `validate_match_expression()`: Set `arms[j].is_reachable =
false` on shadowing.
  - **src/validator.rs:930-993** - `pattern_subsumes()`: Reuse for marking (no changes
needed).

## 3. New Optimizer
  - **src/optimizer.rs** (new): `optimize_matches(program: &mut Program)` - Traverse graphs,
prune `!arm.is_reachable`.

## 4. Codegen Skip
  - **src/codegen.rs:442-513** - `Endpoint::Match` in `process_destination()`: Skip `if
!arm.is_reachable { continue; }`.

## 5. Integration
  - **src/main.rs** (CLI): Add `--optimize` flag, call optimizer post-validate.
  - **src/codegen.rs:807** - `generate_pytorch()`: Run optimizer before emitting.

## Tests
  - **src/validator.rs tests** (~lines 996+): Add shadowing cases.

1. Extend IR for arm metadata (src/ir.rs)
  - Add is_reachable: bool to MatchArm (default true).
  - Update Display/Clone/etc. for new field.
  
<note>**src/ir.rs:129-134** - `MatchArm` struct: Add `is_reachable: bool = true;`</note>
